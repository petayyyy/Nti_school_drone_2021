...
class ArrowDetecting():                                                                                              
    ...
    def color(self,maskk,a,img):
        ...
        if len(contours) != 0:
            ...
            cv2.drawContours(img, [cnt], 0, (193,91,154), 2)
    ...
    def callback(self,data):  
        ...
        if self.Dron_point:
            self.color(cv2.inRange(img, self.green_low, self.green_high),'green',img.copy())
            cv2.putText(img, '!!!!!!!!', (self.cy, self.cx), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 0))
